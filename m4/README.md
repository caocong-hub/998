...
|----------|---------|---------|
| `IEMOCAP_ROOT` | `m4/sample_data/IEMOCAP` | Root folder containing `SessionN/dialog/...` |
| `M4_CHECKPOINT_PATH` | `m4/checkpoints/best_trimodal_model.pth` | Fusion model weights |
| `M4_CORS_ORIGINS` | `http://localhost:3000` | Comma-separated origins for the FastAPI server |

Place your checkpoint file at [`checkpoints/best_trimodal_model.pth`](checkpoints/README.md) or set `M4_CHECKPOINT_PATH` (see [`m4/.env.example`](.env.example)). The repository does **not** ship the `.pth` file; without it, inference uses random classifier weights.
...
`requirements.txt` 已包含 **torch、torchvision、transformers、opencv、librosa、FastAPI** 等运行 `m4_core` + `server.py` 所需的包。
...
Uses the same `IEMOCAP_ROOT` as `m4_core.py`.
